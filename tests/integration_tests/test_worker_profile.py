...
class TestPortfolioImageIntegration:
    ...
    async def test_upload_portfolio_image_success_as_owner(self, async_client: AsyncClient, test_worker_profile: WorkerProfile, auth_headers: dict, monkeypatch: pytest.MonkeyPatch):
        ...
        response = await async_client.post(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images", headers=auth_headers, files={"file": ("portfolio.png", fake_portfolio_image, "image/png")})
        assert response.status_code == 201
        data = response.json()
    ...
    @pytest.mark.integration
    async def test_upload_portfolio_image_success_as_admin(self, async_client: AsyncClient, test_admin_user: User, admin_auth_headers: dict, monkeypatch: pytest.MonkeyPatch, test_worker_profile):
        monkeypatch.setattr("src.app.services.minio_client.minio_client.upload_file", MagicMock(return_value=None))
        fake_portfolio_image = b"\x88PNG\r\n\x1a\n" + b"\x00" * 100
        response = await async_client.post(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images", headers=admin_auth_headers, files={"file": ("portfolio.png", fake_portfolio_image, "image/png")})
        assert response.status_code == 201

    @pytest.mark.integration
    async def test_upload_portfolio_image_forbidden_for_non_owner(self, async_client: AsyncClient, test_worker_profile: WorkerProfile, other_auth_headers: dict, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr("src.app.services.minio_client.minio_client.upload_file", MagicMock(return_value=None))
        fake_portfolio_image = b"\x89PNG\r\n\x1a\n" + b"\x00" * 100  # fake PNG bytes
        response = await async_client.post(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images", headers=other_auth_headers, files={"file": ("portfolio.png", fake_portfolio_image, "image/png")})
        assert response.status_code == 403
    ...
    async def test_upload_portfolio_image_unauthorized_if_not_logged_in(self, async_client: AsyncClient, test_worker_profile: WorkerProfile, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr("src.app.services.minio_client.minio_client.upload_file", MagicMock(return_value=None))
        fake_portfolio_image = b"\x89PNG\r\n\x1a\n" + b"\x00" * 100  # fake PNG bytes
        response = await async_client.post(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images", files={"file": ("portfolio.png", fake_portfolio_image, "image/png")})
        assert response.status_code == 401
    ...
    async def test_upload_portfolio_image_returns_404_for_nonexistent_profile(self, async_client: AsyncClient, monkeypatch: pytest.MonkeyPatch, auth_headers: dict):
        monkeypatch.setattr("src.app.services.minio_client.minio_client.upload_file", MagicMock(return_value=None))
        fake_portfolio_image = b"\x89PNG\r\n\x1a\n" + b"\x00" * 100  # fake PNG bytes
        response = await async_client.post(f"/api/v1/worker-profiles/99999/portfolio-images", headers=auth_headers, files={"file": ("portfolio.png", fake_portfolio_image, "image/png")})
        assert response.status_code == 404
    ...
    async def test_upload_fails_when_max_portfolio_images_reached(self, async_client: AsyncClient, test_worker_profile: WorkerProfile, monkeypatch: pytest.MonkeyPatch, test_portfolio_images: list[PortfolioImage], auth_headers: dict):
        monkeypatch.setattr("src.app.services.minio_client.minio_client.upload_file", MagicMock(return_value=None))
        fake_portfolio_image = b"\x89PNG\r\n\x1a\n" + b"\x00" * 100  # fake PNG bytes
        res = await async_client.post(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images", headers=auth_headers, files={"file": ("portfolio.png", fake_portfolio_image, "image/png")})
    ...
    async def test_get_portfolio_image_success(self, async_client: AsyncClient, test_worker_profile, auth_headers: dict, test_portfolio_images: list[PortfolioImage]):
        ...
        response = await async_client.get(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images/{test_portfolio_images[0].id}", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_portfolio_image_returns_404_if_worker_profile_does_not_exist(self, async_client: AsyncClient, test_portfolio_images: list[PortfolioImage], auth_headers: dict):
        res = await async_client.get(f"/api/v1/worker-profiles/99999/portfolio-images/{test_portfolio_images[0].id}", headers=auth_headers)
    ...
    async def test_get_portfolio_image_returns_404_if_image_does_not_exist(self, async_client: AsyncClient, test_worker_profile: WorkerProfile, auth_headers: dict):
        ...
        response = await async_client.get(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images/99999", headers=auth_headers)
        assert response.status_code == 404
    ...
    async def test_get_portfolio_images_success(self, async_client: AsyncClient, test_worker_profile, test_portfolio_images: list[PortfolioImage], auth_headers):
        ...
        response = await async_client.get(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images", headers=auth_headers)
    ...
    async def test_get_portfolio_images_returns_404_if_worker_profile_does_not_exist(self, async_client: AsyncClient, async_session: AsyncSession, test_worker_profile, auth_headers: dict):
        ...
        response = await async_client.get(f"/api/v1/worker-profiles/99999/portfolio-images", headers=auth_headers)
        assert response.status_code == 404
    ...
    async def test_delete_portfolio_image_returns_404_if_worker_profile_does_not_exist(self, async_client: AsyncClient, test_portfolio_images: list[PortfolioImage], auth_headers: dict):
        ...
        res = await async_client.delete(f"/api/v1/worker-profiles/99999/portfolio-images/{test_portfolio_images[0].id}", headers=auth_headers)
        assert res.status_code == 404
    ...
    async def test_delete_portfolio_image_for_non_owner_return_404(self, async_client: AsyncClient, test_portfolio_images: list[PortfolioImage], auth_headers: dict, test_worker_profile: WorkerProfile):
        ...
        res = await async_client.delete(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images/{non_owner_images[0].id}", headers=auth_headers)
        assert res.status_code == 404
    ...
    async def test_delete_portfolio_image_successful(self, async_client: AsyncClient, test_portfolio_images: list[PortfolioImage], auth_headers: dict, test_worker_profile: WorkerProfile):
        ...
        res = await async_client.delete(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images/{image_to_delete}", headers=auth_headers)
        assert res.status_code == 204
        response = await async_client.get(f"/api/v1/worker-profiles/{test_worker_profile.id}/portfolio-images/{image_to_delete}", headers=auth_headers)
        assert response.status_code == 404
