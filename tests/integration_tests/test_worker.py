...
#         worker = WorkerProfile(user_id=test_user.id)
...
#         }
#
#         worker = WorkerProfile(user_id=test_user.id, bio="Old bio")
#         async_session.add(worker)
#         await async_session.commit()
#
...
#         worker = WorkerProfile(user_id=test_admin_user.id, is_verified=True)
#         async_session.add(worker)
#         await async_session.commit()
#
#         response = await async_client.patch(
...
#         test_worker = WorkerProfile(user_id=test_user.id)
...
#             test_worker: WorkerProfile,
#     ):
#         """Test deleting another user's WorkerProfile profile is forbidden."""
#         response = await async_client.delete(
#             f"/api/v1/worker/{test_worker.id}",
...
#             worker = WorkerProfile(user_id=user.id, is_verified=i % 2 == 0)
#             async_session.add(worker)
#             await async_session.commit()
