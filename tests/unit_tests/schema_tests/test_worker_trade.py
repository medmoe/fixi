...
class TestWorkerTradeSchemas:
    class TestWorkerTradeCreate:
        def test_valid(self):
            schema = WorkerTradeCreate(worker_id=1, trade_id=2, skill_level=SkillLevel.mid)
        ...
        def test_default_skill_level_is_junior(self):
            schema = WorkerTradeCreate(worker_id=1, trade_id=2)
            assert schema.skill_level == SkillLevel.junior

        def test_worker_id_must_be_positive(self):
            with pytest.raises(Exception):
                WorkerTradeCreate(worker_id=0, trade_id=1)

        def test_trade_id_must_be_positive(self):
            with pytest.raises(Exception):
                WorkerTradeCreate(worker_id=1, trade_id=0)

        def test_extra_fields_forbidden(self):
            with pytest.raises(Exception):
                WorkerTradeCreate(worker_id=1, trade_id=2, unexpected="value")

        def test_all_skill_levels_accepted(self):
            for level in SkillLevel:
                schema = WorkerTradeCreate(worker_id=1, trade_id=2, skill_level=level)
