...
def create_schema(**overrides) -> WorkerTradeCreate:
    defaults = {
        "worker_id": 1,  # overridden in tests with real ids
        "trade_id": 1,
...
async def create_test_worker_trade(db: AsyncSession, worker_id: int, trade_id: int, skill_level: SkillLevel = SkillLevel.junior) -> WorkerTrade:
    return await crud_worker_trade.create(
        db=db,
...
async def test_worker_trade(async_session: AsyncSession, test_worker_profile: WorkerProfile, test_trade_category: TradeCategory) -> WorkerTrade:
    return await create_test_worker_trade(
        async_session,
        worker_id=test_worker_profile.id,
...
class TestCreate:
    ...
    async def test_create_persists_fields(
        ...
        assert worker_trade.worker_id == test_worker_profile.id
    ...
    async def test_create_fails_duplicate_worker_trade(self, async_session: AsyncSession, test_worker_profile: WorkerProfile, test_trade_category: TradeCategory):
        await create_test_worker_trade(async_session, worker_id=test_worker_profile.id, trade_id=test_trade_category.id)
...
class TestRead:
    async def test_get_trades_for_worker(
        ...
        result = await crud_worker_trade.get_trades_for_worker(
            db=async_session,
            worker_id=test_worker_profile.id,
        )
        assert len(result) == 1
    ...
    async def test_get_trades_for_worker_fails_nonexistent_worker(
        ...
        with pytest.raises(NoResultFound):
            await crud_worker_trade.get_trades_for_worker(
                db=async_session,
                worker_id=99999,
            )
    ...
    async def test_get_workers_for_trade(
        ...
        result = await crud_worker_trade.get_workers_for_trade(
            db=async_session,
            trade_id=test_trade_category.id,
        )
        assert len(result) == 1
        assert result[0].worker_id == test_worker_profile.id
    ...
    async def test_get_workers_for_trade_fails_nonexistent_trade(
        ...
        with pytest.raises(NoResultFound):
            await crud_worker_trade.get_workers_for_trade(
                db=async_session,
                trade_id=99999,
            )
...
class TestUpdate:
    async def test_update_skill_level(
        ...
        await crud_worker_trade.update(
            db=async_session,
...
class TestDelete:
    async def test_delete_removes_worker_trade(
        ...
        await crud_worker_trade.delete(db=async_session, id=test_worker_trade.id)
    ...
    async def test_delete_fails_nonexistent_worker_trade(
        ...
        with pytest.raises(NoResultFound):
            await crud_worker_trade.delete(db=async_session, id=99999)
...
class TestEdgeCases:
    ...
    async def test_reassign_trade_after_deletion(
        ...
        assert new_trade.worker_id == test_worker_profile.id
