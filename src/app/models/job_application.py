...
class JobApplication(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("job_id", "worker_profile_id", name="unique_job_application"),
        # Powers analytics date-range/status filtering (Phase 8 Issue 8) --
    ...
    status: Mapped[ApplicationStatus] = mapped_column(
        SAEnum(ApplicationStatus, values_callable=lambda v: [e.value for e in v]),
        nullable=False, default=ApplicationStatus.PENDING, index=True,
    )
