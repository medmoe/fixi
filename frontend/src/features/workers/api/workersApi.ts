...
import type { WorkerNearbyRead, WorkerPublicRead, WorkerRatingSummary, WorkerReview, WorkersResponse } from '../types';
...
export const workersApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getNearbyWorkers: builder.query<WorkerNearbyRead[], { latitude: number; longitude: number; radius_km?: number }>({
